...
def validate_research_data(data_source: str, result_json: str) -> str:
    ...
    try:
        ...
        if not isinstance(data, dict):
            return json.dumps({"is_valid": False, "completeness_score": 0.0, "missing_fields": ["all"], "suggestions": "Expected JSON object, got array or other type."})
    except (ValueError, TypeError):
        return json.dumps({"is_valid": False, "completeness_score": 0.0, "missing_fields": ["all"], "suggestions": "Invalid JSON. Retry the tool call."})
    ...
    if data_source == "corp_info":
        ...
        return json.dumps({"is_valid": score >= 0.4, "completeness_score": round(score, 2), "missing_fields": missing, "suggestions": "Proceed." if score >= 0.4 else "Data sparse. Try alternative name."})
    elif data_source == "financial_info":
        key_fields = ["revenue", "operating_profit", "net_income", "total_assets"]
        present = [f for f in key_fields if data.get(f)]
        missing = [f for f in key_fields if not data.get(f)]
        score = len(present) / len(key_fields)
        return json.dumps({"is_valid": score >= 0.25, "completeness_score": round(score, 2), "missing_fields": missing, "suggestions": "Proceed." if score >= 0.25 else "Financial data unavailable. Skip."})
    elif data_source == "news":
        articles = data.get("articles", [])
        score = min(len(articles) / 5, 1.0)
        return json.dumps({"is_valid": len(articles) > 0, "completeness_score": round(score, 2), "missing_fields": [] if articles else ["articles"], "suggestions": f"{len(articles)} articles." if articles else "No news. Proceed without."})
    else:
        return json.dumps({"is_valid": bool(data), "completeness_score": 0.5, "missing_fields": [], "suggestions": "Unknown source."})
