...
export default function CoverLetterPage({ onNavigate }: Props) {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <div className="mb-6">
        <h1 className="text-xl font-bold text-zinc-900">Cover Letter Engine</h1>
        <p className="text-sm text-zinc-500 mt-1">JD, 이력서, 기업 리서치 기반으로 맞춤형 자소서를 생성합니다.</p>
      </div>

      {/* 준비 상태 — 4개 배지 */}
      <div className="flex flex-wrap gap-2 mb-5">
        {[
          { label: 'JD', ready: !!jd, detail: jd ? jd.company : null, page: 'jd-match' as LayoutPageKey, required: true },
      ...
      {!canRun && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex items-start gap-2 mb-5">
          ...
          <p className="text-sm text-amber-800">
            JD Match 페이지에서 JD 파싱과 이력서 업로드를 완료해야 자소서를 생성할 수 있습니다.
          </p>
        </div>
      )}
      ...
      {canRun && !result && (
        <div className="bg-white border border-zinc-200 rounded-xl p-5 mb-4">
          <h3 className="text-sm font-semibold text-zinc-800 mb-3">생성 정보 확인</h3>
          <div className="flex gap-4 text-sm text-zinc-600">
            <div>
              <span className="text-xs text-zinc-400">대상 기업</span>
              <p className="font-medium text-zinc-900 mt-0.5">{jd?.company ?? '—'}</p>
            </div>
            <div className="w-px bg-zinc-100" />
            <div>
              <span className="text-xs text-zinc-400">포지션</span>
              <p className="font-medium text-zinc-900 mt-0.5">{jd?.title ?? '—'}</p>
            </div>
            <div className="w-px bg-zinc-100" />
            <div>
              <span className="text-xs text-zinc-400">지원자</span>
              <p className="font-medium text-zinc-900 mt-0.5">{resume?.personal_info?.name ?? '—'}</p>
            </div>
            {matchResult && (
              <>
                <div className="w-px bg-zinc-100" />
                <div>
                  ...
                  <p className="font-medium text-lime-600 mt-0.5">{matchResult.ats_score.toFixed(0)}</p>
                </div>
              </>
            )}
          </div>
      ...
      >
        <PenLine size={15} />
        {loading ? 'AI 자소서 작성 중...' : result ? '다시 생성' : '자소서 생성'}
      </button>
      ...
      {loading && (
        <div className="bg-white border border-zinc-200 rounded-xl p-10 text-center">
          <PenLine size={24} className="mx-auto text-lime-600 animate-pulse mb-3" />
          <p className="text-sm font-medium text-zinc-700">자소서 작성 중...</p>
          <p className="text-xs text-zinc-400 mt-1">JD 키워드 · 이력서 · 기업 정보를 분석해 맞춤형 내용을 작성합니다</p>
        </div>
      )}
      ...
      {result && !loading && (
        <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              { label: '키워드 커버리지', value: `${(result.jd_keyword_coverage * 100).toFixed(0)}%`, color: 'text-lime-600' },
              { label: '글자 수', value: result.word_count.toLocaleString(), color: 'text-zinc-800' },
              { label: '핵심 포인트', value: result.key_points?.length ?? 0, color: 'text-zinc-800' },
          ...
          {result.key_points && result.key_points.length > 0 && (
            <div className="bg-white border border-zinc-200 rounded-xl p-5">
              <h3 className="text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-3">전략적 핵심 포인트</h3>
          ...
          <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-zinc-100">
              <h3 className="text-sm font-semibold text-zinc-900">자기소개서</h3>
              <button
                onClick={handleCopy}
              ...
              >
                {copied ? <CheckCheck size={12} className="text-lime-600" /> : <Copy size={12} />}
                {copied ? '복사됨' : '복사'}
              </button>
            </div>
