...
import { Upload, BarChart2, CheckCircle2, XCircle, ArrowRight, RefreshCw } from 'lucide-react'
...
export default function JDMatchPage({ userId, onNavigate }: Props) {
  const { jd, setJd, resume, setResume, matchResult, setMatchResult } = useAppContext()
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <div className="mb-6">
        <h1 className="text-xl font-bold text-zinc-900">JD-Resume Match</h1>
        <p className="text-sm text-zinc-500 mt-1">채용공고와 이력서를 분석해 ATS 매칭 점수를 계산합니다.</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        ...
        <div className="bg-white border border-zinc-200 rounded-xl p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm font-semibold text-zinc-900">채용공고 URL</h3>
            {jd && (
              <button onClick={() => setJd(null)} className="ml-auto text-xs text-zinc-400 hover:text-zinc-600">
                <RefreshCw size={12} />
              </button>
            )}
          </div>
              ...
              {Array.isArray(jd.skills) && jd.skills.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-2">
                  {(jd.skills as string[]).slice(0, 5).map(s => (
                    <span key={s} className="text-xs px-1.5 py-0.5 bg-white border border-lime-200 text-zinc-600 rounded">
                      {s}
                    </span>
                  ))}
                  {jd.skills.length > 5 && (
                    <span className="text-xs text-zinc-400">+{jd.skills.length - 5}</span>
                  )}
                </div>
              )}
            </div>
        ...
        <div className="bg-white border border-zinc-200 rounded-xl p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm font-semibold text-zinc-900">이력서 업로드</h3>
            {resume && (
              <button onClick={() => setResume(null)} className="ml-auto text-xs text-zinc-400 hover:text-zinc-600">
                <RefreshCw size={12} />
              </button>
            )}
          </div>
            ...
            <div className="flex flex-col gap-2">
              <label className="flex flex-col items-center justify-center gap-2 p-5 border-2 border-dashed
                ...
                <input
                  ...
                  onChange={e => { setResume(null); setResumeFile(e.target.files?.[0] ?? null) }}
                />
              </label>
              <button
              ...
              <div className="flex-1 grid grid-cols-2 gap-4">
                <div>
                  ...
                  {matchResult.strengths.slice(0, 3).map((s, i) => (
                    <p key={i} className="text-xs text-zinc-600 mb-1 leading-relaxed">· {s}</p>
                  ))}
                </div>
                <div>
                  <h4 className="text-xs font-semibold text-zinc-700 mb-2 flex items-center gap-1">
                  ...
                  {matchResult.weaknesses.slice(0, 3).map((w, i) => (
                    <p key={i} className="text-xs text-zinc-600 mb-1 leading-relaxed">· {w}</p>
                  ))}
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white border border-zinc-200 rounded-xl p-4">
              <h4 className="text-xs font-semibold text-emerald-700 mb-3">
                ✅ 매칭 키워드 <span className="text-zinc-400 font-normal">({matchResult.matched_keywords.length})</span>
              </h4>
              <div className="flex flex-wrap gap-1.5">
                {matchResult.matched_keywords.map(k => (
                  <span key={k} className="text-xs px-2 py-0.5 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-full">
                    {k}
                  </span>
                ))}
              </div>
            ...
            <div className="bg-white border border-zinc-200 rounded-xl p-4">
              <h4 className="text-xs font-semibold text-red-600 mb-3">
                ⚠️ 누락 키워드 <span className="text-zinc-400 font-normal">({matchResult.missing_keywords.length})</span>
              </h4>
              <div className="flex flex-wrap gap-1.5">
                {matchResult.missing_keywords.map(k => (
                  <span key={k} className="text-xs px-2 py-0.5 bg-red-50 text-red-600 border border-red-200 rounded-full">
                    {k}
                  </span>
                ))}
              </div>
