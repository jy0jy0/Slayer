...
export default function OptimizePage({ onNavigate }: Props) {
  const { jd, resume, matchResult, setOptimizeResult, optimizeResult } = useAppContext()
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <div className="mb-6">
        <h1 className="text-xl font-bold text-zinc-900">Resume Optimize</h1>
        <p className="text-sm text-zinc-500 mt-1">이력서를 반복적으로 개선해 목표 ATS 점수에 도달합니다.</p>
      </div>

      {/* 준비 상태 */}
      <div className="grid grid-cols-3 gap-2 mb-5">
        {[
          { label: 'JD', ready: !!jd, detail: jd ? `${jd.company ?? '—'} · ${jd.title ?? '—'}` : undefined, page: 'jd-match' as LayoutPageKey },
          { label: '이력서', ready: !!resume, detail: resume?.personal_info?.name ?? undefined, page: 'jd-match' as LayoutPageKey },
          { label: '매칭 결과', ready: !!matchResult, detail: matchResult ? `ATS ${matchResult.ats_score.toFixed(0)}점` : undefined, page: 'jd-match' as LayoutPageKey },
          ...
          >
            <div className="flex items-center gap-1.5 mb-0.5">
              <span className={`text-xs ${ready ? 'text-lime-600' : 'text-zinc-400'}`}>
                {ready ? '✓' : '○'}
              </span>
              <span className={`text-xs font-semibold ${ready ? 'text-lime-800' : 'text-zinc-500'}`}>
                {label}
              </span>
            </div>
            {ready && detail ? (
      ...
      {!canRun && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex items-start gap-2 mb-5">
          ...
          <p className="text-sm text-amber-800">
            JD Match 페이지에서 JD 파싱 + 이력서 업로드 + 매칭 분석을 먼저 완료하세요.
          </p>
        </div>
      )}
      ...
      <div className="bg-white border border-zinc-200 rounded-xl p-5 mb-4">
        <h3 className="text-sm font-semibold text-zinc-800 mb-4">최적화 설정</h3>
        <div className="grid grid-cols-2 gap-6">
          <div>
            <label className="text-xs text-zinc-500 block mb-2">
              목표 점수 <b className="text-zinc-900 text-sm ml-1">{targetScore}</b>
            </label>
            <input
          ...
          <div>
            <label className="text-xs text-zinc-500 block mb-2">
              최대 반복 <b className="text-zinc-900 text-sm ml-1">{maxIter}회</b>
            </label>
      ...
      {loading && (
        <div className="bg-white border border-zinc-200 rounded-xl p-10 text-center">
          <Sparkles size={24} className="mx-auto text-lime-600 animate-pulse mb-3" />
          <p className="text-sm font-medium text-zinc-700">이력서 최적화 중...</p>
          <p className="text-xs text-zinc-400 mt-1">최대 {maxIter}회 반복합니다</p>
        </div>
      )}
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="bg-white border border-zinc-200 rounded-xl p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-sm font-semibold text-zinc-800">최적화 결과</h3>
            </div>
            <div className="grid grid-cols-3 gap-4 text-center mb-4">
              <div className="p-3 bg-zinc-50 rounded-lg">
                <p className="text-xs text-zinc-400 mb-1">반복 횟수</p>
                <p className="text-2xl font-bold text-zinc-800">{optimizeResult.iterations_used}</p>
              </div>
              <div className="p-3 bg-lime-50 rounded-lg">
                <p className="text-xs text-zinc-400 mb-1">최종 점수</p>
                <p className="text-2xl font-bold text-lime-600">{optimizeResult.final_ats_score.toFixed(0)}</p>
              </div>
              <div className="p-3 bg-emerald-50 rounded-lg">
                <p className="text-xs text-zinc-400 mb-1">향상</p>
                <p className="text-2xl font-bold text-emerald-600">+{optimizeResult.score_improvement.toFixed(0)}</p>
              </div>
            </div>
          ...
          {optimizeResult.changes && optimizeResult.changes.length > 0 && (
            <div className="bg-white border border-zinc-200 rounded-xl p-5">
              <h3 className="text-sm font-semibold text-zinc-800 mb-3">변경사항 ({optimizeResult.changes.length}건)</h3>
                ...
                {optimizeResult.changes.map((change, i) => (
                  <div key={i} className="border border-zinc-100 rounded-lg p-3">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      {change.field && <span className="text-xs text-zinc-400">{change.field}</span>}
                    </div>
                    {change.before && change.after && (
                      <div className="flex gap-2 items-start text-xs">
                        ...
                        <ArrowRight size={11} className="text-zinc-300 mt-2 shrink-0" />
                    ...
                    {change.reason && (
                      <p className="text-xs text-zinc-400 mt-2">💡 {change.reason}</p>
                    )}
                  </div>
                ))}
              </div>
          ...
          <div className="bg-zinc-50 border border-zinc-200 rounded-xl p-4">
            ...
            <div className="flex gap-2">
              {[
                { page: 'cover-letter' as LayoutPageKey, label: '자소서 생성', desc: '최적화된 이력서 반영' },
                { page: 'interview-prep' as LayoutPageKey, label: '면접 준비', desc: '개선된 프로필 기반' },
              ].map(({ page, label, desc }) => (
                <button
                  key={page}
                ...
                >
                  <p className="text-xs font-semibold text-zinc-900 group-hover:text-lime-700">{label}</p>
