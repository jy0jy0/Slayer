...
import { MessageCircleQuestion, ChevronDown, ChevronUp, AlertCircle } from 'lucide-react'
...
function CategoryAccordion({ category, questions }: {
  category: string
    ...
    <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-zinc-400 font-normal">({category})</span>
        ...
        <div className="border-t border-zinc-100 divide-y divide-zinc-100">
          {questions.map((q, i) => (
            <div key={i} className="px-5 py-4">
              <p className="text-sm font-medium text-zinc-900 mb-2 leading-relaxed">Q{i + 1}. {q.question}</p>
              {q.intent && (
...
export default function InterviewPrepPage({ onNavigate }: Props) {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <div className="mb-6">
        <h1 className="text-xl font-bold text-zinc-900">Interview Prep</h1>
        <p className="text-sm text-zinc-500 mt-1">JD와 이력서 기반으로 카테고리별 면접 예상 질문을 생성합니다.</p>
      </div>

      {/* 준비 상태 */}
      <div className="flex flex-wrap gap-2 mb-5">
        {[
          { label: 'JD', ready: !!jd, detail: jd?.company, page: 'jd-match' as LayoutPageKey, required: true },
          { label: '이력서', ready: !!resume, detail: resume?.personal_info?.name, page: 'jd-match' as LayoutPageKey, required: true },
          { label: '기업 리서치', ready: !!companyResearch, detail: companyResearch?.company_name, page: 'company-research' as LayoutPageKey, required: false },
          { label: '매칭 결과', ready: !!matchResult, detail: matchResult ? `ATS ${matchResult.ats_score.toFixed(0)}` : null, page: 'jd-match' as LayoutPageKey, required: false },
          ...
          >
            ...
            <span>{label}</span>
            {ready && detail && <span className="text-zinc-400 font-normal">— {detail}</span>}
          </button>
        ))}
      </div>
      ...
      {!canRun && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex items-start gap-2 mb-5">
          ...
          <p className="text-sm text-amber-800">
            JD Match 페이지에서 JD 파싱과 이력서 업로드를 완료해야 면접 질문을 생성할 수 있습니다.
          </p>
        </div>
      )}
      ...
      <div className="bg-white border border-zinc-200 rounded-xl p-5 mb-4">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-semibold text-zinc-800">카테고리당 질문 수</p>
            <p className="text-xs text-zinc-400 mt-0.5">기술 · 경험 · 상황/행동 · 인성 · 컬처핏 · 기업 이해도</p>
          </div>
      ...
      {loading && (
        <div className="bg-white border border-zinc-200 rounded-xl p-10 text-center">
          <MessageCircleQuestion size={24} className="mx-auto text-lime-600 animate-pulse mb-3" />
          <p className="text-sm font-medium text-zinc-700">면접 질문 생성 중...</p>
          <p className="text-xs text-zinc-400 mt-1">6개 카테고리 × {qPerCategory}개 = 최대 {6 * qPerCategory}개 질문</p>
        </div>
      )}

      {result && !loading && (
        <div className="flex flex-col gap-4">
          {result.excluded_categories && result.excluded_categories.length > 0 && (
            <div className="bg-zinc-50 border border-zinc-200 rounded-xl px-4 py-3 text-sm text-zinc-600">
              데이터 부족으로 제외된 카테고리: {result.excluded_categories.join(', ')}
            </div>
          )}
          ...
          <div className="bg-white border border-zinc-200 rounded-xl px-5 py-4 flex items-center justify-between">
            <p className="text-sm text-zinc-700">
              총 <b className="text-zinc-900 text-base">{result.questions.length}</b>개 면접 질문 생성됨
            </p>
            <div className="flex gap-1.5">
              {Object.keys(byCategory).map(cat => {
                const [emoji] = CATEGORY_LABELS[cat] ?? ['❓']
                return (
                  <span key={cat} className="text-base" title={cat}>{emoji}</span>
                )
              })}
            </div>
          </div>
          ...
          {result.weak_areas && result.weak_areas.length > 0 && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-5">
              <h3 className="text-sm font-semibold text-amber-900 mb-2">⚠️ 집중 준비 영역</h3>
              {result.weak_areas.map((area, i) => (
                <p key={i} className="text-sm text-amber-800 leading-relaxed">· {area}</p>
              ))}
            </div>
          )}
          ...
          {result.sample_answers && result.sample_answers.length > 0 && (
            <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden">
              <div className="px-5 py-3.5 border-b border-zinc-100">
                <h3 className="text-sm font-semibold text-zinc-800">예시 답변</h3>
              </div>
                ...
                {result.sample_answers.map((sa, i) => (
                  <details key={i} className="group">
                    <summary className="flex items-center gap-2 px-5 py-3.5 cursor-pointer hover:bg-zinc-50 transition-colors list-none">
                      <ChevronDown size={13} className="text-zinc-400 group-open:rotate-180 transition-transform shrink-0" />
                      <span className="text-sm text-zinc-800">💬 {sa.question}</span>
                    </summary>
