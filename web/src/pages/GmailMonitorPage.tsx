...
export default function GmailMonitorPage({ userId }: Props) {
  ...
  const [pollResult, setPollResult] = useState<{ processed: number } | null>(null)
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <div className="mb-6">
        <h1 className="text-xl font-bold text-zinc-900">Gmail Monitor</h1>
        <p className="text-sm text-zinc-500 mt-1">채용 관련 메일을 자동 감지하고 지원 상태를 업데이트합니다.</p>
      </div>
      ...
      <div className="bg-white border border-zinc-200 rounded-xl p-5 mb-4">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-zinc-900">새 채용 메일 확인</p>
            <p className="text-xs text-zinc-500 mt-0.5">마지막 확인 이후 수신된 채용 메일을 분류합니다.</p>
          </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          {events.length === 0 ? (
            <div className="bg-white border border-zinc-200 rounded-xl p-8 text-center">
              <p className="text-sm text-zinc-500">감지된 채용 관련 메일이 없습니다.</p>
            </div>
          ) : (
            <>
              ...
              {events.map(ev => {
                const meta = STATUS_META[ev.status_type ?? ''] ?? { icon: '📧', label: ev.status_type ?? '', color: '#a1a1aa' }
                return (
                  <div key={ev.id} className="bg-white border border-zinc-200 rounded-xl p-5 hover:shadow-sm transition-all">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {ev.stage && (
                            <span className="text-xs text-zinc-500">— {ev.stage}</span>
                          )}
                        </div>
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          {ev.received_at && (
                            <span className="text-xs text-zinc-400">
                              {new Date(ev.received_at).toLocaleDateString('ko-KR', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                            </span>
                          )}
                        </div>
                        {ev.subject && (
                          <p className="text-sm text-zinc-700 mb-1">{ev.subject}</p>
                        )}
                        {ev.summary && (
                          <p className="text-xs text-zinc-500 leading-relaxed">{ev.summary}</p>
                        )}
                      </div>
                    </div>
                  </div>
      ...
      {!loaded && (
        <div className="text-center py-16 text-zinc-400">
          ...
          <p className="text-xs mt-1 text-zinc-400">Gmail 연동이 필요합니다 (Google OAuth 로그인)</p>
        </div>
      )}
    </div>
  )
